use crate::{
    logging,
    problem::{
        Problem, ProblemAnswerSource, ProblemReviewConfidence, ProblemReviewStatus,
        ProblemReviewVerdict, visible_tags,
    },
    store::AnswerRecord,
};
...
pub enum AnalysisStreamEvent {
    ...
    ExplanationsGenerated(Vec<Problem>),
...
const PREFETCHED_EXPLANATION_PREFIX: &str = "AI预生成解析：";
...
pub fn annotate_problem_knowledge_points_with_progress(
    ...
    let concurrency = config.knowledge_point_concurrency.clamp(1, 256);
...
pub fn pre_generate_explanations_with_progress(
    config: &AiConfig,
    problems: Vec<Problem>,
    on_progress: Option<&dyn Fn(usize, usize)>,
) -> Vec<Problem> {
    let pending_total = problems
        .iter()
        .filter(|problem| should_pre_generate_explanation(problem))
        .count();
    if pending_total == 0 || !config.enabled || config.endpoint.trim().is_empty() {
        if let Some(callback) = on_progress {
            callback(pending_total, pending_total);
    ...
    let concurrency = config.knowledge_point_concurrency.clamp(1, 256);
    let mut generated = Vec::with_capacity(problems.len());
    let mut handles = Vec::new();
    let mut completed = 0usize;
    ...
    for problem in problems {
        if !should_pre_generate_explanation(&problem) {
        ...
        let mut task_config = config.clone();
        task_config.model = model.clone();
        let handle = std::thread::spawn(move || pre_generate_one_explanation(task_config, problem));
        handles.push(handle);
        if handles.len() >= concurrency {
            completed += join_problem_handles(&mut handles, &mut generated);
            if let Some(callback) = on_progress {
                callback(completed, pending_total);
            }
        }
...
fn should_pre_generate_explanation(problem: &Problem) -> bool {
    !problem.needs_ai_review()
        && !problem.answer.trim().is_empty()
        && !problem
            .explanation
            .trim_start()
            .starts_with(PREFETCHED_EXPLANATION_PREFIX)
}

fn pre_generate_one_explanation(config: AiConfig, mut problem: Problem) -> Problem {
    match send_prompt(&config, build_prefetch_explanation_prompt(&problem)) {
        Ok(text) => {
            let explanation = text.trim();
            if !explanation.is_empty() {
                problem.explanation =
                    merge_prefetched_explanation(explanation, &problem.explanation);
            }
        }
        Err(err) => log::warn!(
            "AI explanation pre-generation failed: problem_id={}, error={err}",
            problem.id
        ),
    }
    problem
...
fn merge_prefetched_explanation(generated: &str, existing: &str) -> String {
    let generated = generated.trim();
    let existing = existing.trim();
    if existing.is_empty() || existing.starts_with(PREFETCHED_EXPLANATION_PREFIX) {
        format!("{PREFETCHED_EXPLANATION_PREFIX}\n{generated}")
    } else {
        format!(
            "{PREFETCHED_EXPLANATION_PREFIX}\n{generated}\n\n---\n原解析/复核记录：\n{existing}"
        )
    }
}
...
fn apply_structured_review(problem: &mut Problem, review: StructuredReviewResponse) {
    let ai_verdict = review
        ...
        .unwrap_or("无法判断");
    let confidence_text = review
        .confidence
        .as_deref()
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .unwrap_or("未知");
    let confidence_level = classify_review_confidence(confidence_text);
    let ai_explanation = review
        .explanation
        .as_deref()
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .unwrap_or("AI 已完成复核，但没有返回详细理由。");
    let history_answer = history_answer_for_review(problem);
    let reference_answer = review
        .reference_answer
        .as_deref()
        .map(str::trim)
        .and_then(|value| normalize_reference_answer(problem, value));
    let verdict = reference_answer
        .as_deref()
        .and_then(|reference| compare_history_and_reference(problem, &history_answer, reference))
        .unwrap_or_else(|| normalize_verdict(ai_verdict));

    clear_ai_review_tags(&mut problem.tags);
    let conflicts_with_inferred_wrong = reference_answer
        .as_deref()
        .and_then(|reference| compare_history_and_reference(problem, &history_answer, reference))
    ...
    let auto_accept = reference_answer.is_some()
        && confidence_level != ProblemReviewConfidence::Low
        && verdict != "无法判断"
        && !conflicts_with_inferred_wrong;

    problem.state.review_verdict = verdict_to_state(verdict);
    problem.state.review_confidence = confidence_level;
    if problem.state.user_answer.trim().is_empty() {
        problem.state.user_answer = history_answer.clone();
    }

    if auto_accept {
        if let Some(answer) = reference_answer.as_ref() {
            problem.answer = answer.clone();
        }
        problem.state.answer_source = ProblemAnswerSource::AiReviewed;
        ...
        problem.state.review_status = if conflicts_with_inferred_wrong {
    ...
    let adoption_note = if auto_accept {
        "已采用 AI 核查给出的参考答案。"
    } else if conflicts_with_inferred_wrong {
        "AI 给出的参考答案与题目已知作答结果冲突，本次未自动采用，保留待复核状态。"
    } else if reference_answer.is_none() {
        "AI 未提供可用参考答案，本次未自动采用，保留待复核状态。"
    } else if confidence_level == ProblemReviewConfidence::Low {
        "AI 置信度偏低，本次未自动采用，保留待复核状态。"
    } else {
        "AI 结果不满足自动采纳条件，本次未自动采用，保留待复核状态。"
    };

    problem.explanation = format!(
        "AI复核结果：{verdict}\n置信度：{confidence_text}\n\n{adoption_note}\n\n{ai_explanation}"
    );
}

fn classify_review_confidence(text: &str) -> ProblemReviewConfidence {
    let value = text.trim();
    if value.contains('高') {
...
fn verdict_to_state(verdict: &str) -> ProblemReviewVerdict {
    match verdict {
        "正确" => ProblemReviewVerdict::Correct,
        "错误" => ProblemReviewVerdict::Wrong,
        _ => ProblemReviewVerdict::Unknown,
    }
}

fn history_answer_for_review(problem: &Problem) -> String {
    if problem.state.user_answer.trim().is_empty() {
        problem.answer.clone()
    } else {
        problem.state.user_answer.clone()
    }
}

fn normalize_reference_answer(problem: &Problem, value: &str) -> Option<String> {
    ...
    match problem.kind() {
        crate::problem::ProblemType::SingleChoice => {
            let normalized = crate::problem::normalize_choice_answer(value);
            if normalized.chars().count() == 1 && is_valid_choice_answer(problem, &normalized) {
                Some(normalized)
            } else {
                None
            }
        ...
        crate::problem::ProblemType::MultipleChoice => {
            let normalized = crate::problem::normalize_choice_answer(value);
            if !normalized.is_empty() && is_valid_choice_answer(problem, &normalized) {
                Some(normalized)
            } else {
                None
            }
        }
        crate::problem::ProblemType::Text => Some(value.to_owned()),
    }
}

fn is_valid_choice_answer(problem: &Problem, answer: &str) -> bool {
    let option_keys = problem
        .options()
        .into_iter()
        .filter_map(|option| option.key.chars().next())
        .collect::<std::collections::BTreeSet<_>>();
    !option_keys.is_empty() && answer.chars().all(|ch| option_keys.contains(&ch))
}

fn compare_history_and_reference(
    problem: &Problem,
    history_answer: &str,
    reference_answer: &str,
) -> Option<&'static str> {
    match problem.kind() {
        crate::problem::ProblemType::SingleChoice | crate::problem::ProblemType::MultipleChoice => {
            let history = crate::problem::normalize_choice_answer(history_answer);
            let reference = crate::problem::normalize_choice_answer(reference_answer);
            if history.is_empty() || reference.is_empty() {
                None
            } else if history == reference {
                Some("正确")
            } else {
                Some("错误")
            }
        }
        crate::problem::ProblemType::Text => {
            let history = normalize_text_answer(history_answer);
            let reference = normalize_text_answer(reference_answer);
            if history.is_empty() || reference.is_empty() {
                None
            } else if history == reference {
                Some("正确")
...
fn normalize_verdict(verdict: &str) -> &'static str {
    let value = verdict.trim();
    if value.contains("正确") {
        "正确"
...
fn normalize_text_answer(value: &str) -> String {
    value
        .split_whitespace()
        .collect::<Vec<_>>()
        .join(" ")
...
fn clear_ai_review_tags(tags: &mut Vec<String>) {
    tags.retain(|tag| {
...
fn build_structured_review_prompt(problem: &Problem) -> String {
    format!(
        "你是 ShuaForge 的题目复核助手，不要自称老师。页面没有提供标准答案，题库中保存的 answer 是用户历史作答，可能正确也可能错误。请根据题干与选项独立推断标准答案，再判断历史作答是否正确。\n\n必须只输出严格 JSON，不要 Markdown，不要代码块。\n输出格式：{{\"verdict\":\"正确/错误/无法判断\",\"reference_answer\":\"可确认的标准答案；无法确认则为空字符串\",\"explanation\":\"简要理由\",\"confidence\":\"高/中/低\"}}\n\n规则：\n0. verdict 字段表示“历史作答记录是否正确”，不是题目陈述本身真伪。\n1. 单选/判断题 reference_answer 只输出一个选项字母，例如 A。\n2. 多选题 reference_answer 输出全部选项字母并按字母升序排列，例如 AC；不确定完整答案则留空。\n3. 填空/主观题只有在题干能明确推出答案时才填写，否则留空。\n4. 历史作答仅用于对照，不可作为推断标准答案的依据。\n5. 只要不能高置信度确认标准答案，就返回 verdict=无法判断 且 reference_answer 为空字符串。宁可无法判断，不要猜测。\n\n题型：{:?}\n题目：{}\n\n历史作答记录（仅供参考，可能错误）：{}\n原解析/导出说明：{}",
        problem.kind(),
        problem.prompt,
        history_answer_for_review(problem),
...
fn build_prefetch_explanation_prompt(problem: &Problem) -> String {
    format!(
        "你是 ShuaForge 的刷题解析助手，不要自称老师。请为这道题预先生成一份简洁但完整的中文解析，供用户答错后即时查看。\n\n要求：\n1. 可以直接给出标准答案。\n2. 说明为什么标准答案成立。\n3. 如果是选择题，简要说明常见干扰项为什么不对。\n4. 给出一个记忆/审题要点。\n5. 不要编造题干没有的信息；无法充分解释时说明依据有限。\n\n题目：{}\n\n标准答案：{}\n\n已有解析/复核记录：{}",
        problem.prompt,
        problem.answer,
        if problem.explanation.trim().is_empty() {
            "无"
...
fn compact_problem_set_analysis_system_prompt() -> String {
    "你是 ShuaForge 的 AI 总监，职责是审查题库采集质量、复核低置信题目，并把本次分析中已经执行的处理动作讲清楚；不要自称老师。程序已经在本地读取完全部题目，并先执行了确定性清洗与待复核题 AI 复核，随后提供全量统计、状态字段、本次处理摘要、AI 复核写回明细和压缩逐题摘要。请直接输出最终 Markdown 处理报告。\n\n强制规则：\n- 明确写出“数据读取范围：已读取全部题目”。\n- 必须优先说明“本次分析已处理了什么”：例如已写回高置信 AI 参考答案、已保留冲突题、已标记人工确认。\n- AI 复核写回题目的题号、题干和答案只能来自用户消息中的“AI复核写回明细”；如果明细为“无”，不要编造具体写回题目。\n- 高置信且通过安全条件的 AI 复核参考答案会写回题库数据库；冲突、低置信或无证据的复核结果不会写回。\n- 题型数量、标签频次、answer_source/review 状态必须使用用户消息中的全量统计，不要重新猜测。\n- 不允许凭空补标准答案；没有证据的题只能标记为待复核/需人工确认。\n- 不要只把问题抛给用户。凡是本次分析已能处理的，写成“已处理”；程序不能安全处理的，写清“为什么不能自动处理”和“用户唯一需要做的动作”。\n- 对“无标准答案/按得分推断/AI 已复核但冲突/待人工确认”的数据质量限制要单独说明。\n- 难度、易混点、刷题顺序只能基于压缩摘要做审慎观察，不能编造不存在的题目。\n- 输出结构必须包含：处理结论、已自动处理、仍需人工确认、数据质量风险、下一步建议。".to_owned()
}
...
fn build_compact_problem_set_analysis_prompt(title: &str, problems: &[Problem]) -> String {
    ...
    let action_summary = director_action_summary(problems);
    let review_acceptance_details = ai_review_acceptance_details(problems, 20);
    let examples = compact_problem_examples(problems, 80);

    format!(
        "题组/题库「{title}」已经由程序本地读取完全部题目，并交给 AI 总监做导入质量与学习价值审查。请直接生成最终处理报告。\n\n数据读取范围：已读取全部题目（{} / {}）。\n\n题库总览：\n{stats}\n\n得分/复核摘要：\n{score_summary}\n\n程序处理动作摘要：\n{action_summary}\n\nAI复核写回明细（程序确定生成，报告里的具体写回题目只能来自这里）：\n{review_acceptance_details}\n\n压缩逐题摘要（最多 80 条，优先覆盖待复核/冲突样例；完整统计以上方总览为准）：\n{examples}",
        problems.len(),
        problems.len()
    )
}

fn director_action_summary(problems: &[Problem]) -> String {
    let mut ai_accepted = 0usize;
    let mut pending = 0usize;
    let mut conflict = 0usize;
    let mut unknown = 0usize;
    let mut score_inferred = 0usize;
    let mut user_temporary = 0usize;
    ...
    for problem in problems {
        ...
        if problem.state.review_status == ProblemReviewStatus::Conflict {
            conflict += 1;
        }
        if problem.state.review_status == ProblemReviewStatus::Unknown {
            unknown += 1;
        }
        if problem.state.answer_source == ProblemAnswerSource::ScoreInferred {
            score_inferred += 1;
        }
        if problem.state.answer_source == ProblemAnswerSource::UserTemporary {
            user_temporary += 1;
        }
    }

    format!(
        "- 已写回高置信 AI 复核参考答案：{ai_accepted}\n- 仍需人工确认：{pending}\n- 其中 AI/得分或证据冲突：{conflict}\n- 其中 AI 无法确认：{unknown}\n- 按得分推断保留：{score_inferred}\n- 仅有用户临时答案：{user_temporary}\n- 处理原则：仅写回高置信且通过安全条件的 AI 参考答案；冲突、低置信或无证据的题不会自动篡改标准答案，只保留复核状态。"
    )
}

fn ai_review_acceptance_details(problems: &[Problem], limit: usize) -> String {
    let total = problems
        .iter()
        .filter(|problem| {
            problem.state.answer_source == ProblemAnswerSource::AiReviewed
    ...
    let mut rows = problems
        ...
        .map(|(index, problem)| {
            format!(
                "- 第 {} 题「{}」，已写回 AI 复核答案 {}",
                index + 1,
                summarize_for_prompt(&problem.question_text(), 48),
                summarize_for_prompt(&problem.answer, 16)
            )
        })
        .collect::<Vec<_>>();

    if total > limit {
        rows.push(format!("- 其余 {} 题略。", total - limit));
    }
    if rows.is_empty() {
        "无".into()
    } else {
...
fn score_inference_summary(problems: &[Problem]) -> String {
    ...
    for problem in problems {
        ...
        if !problem.state.score_display.trim().is_empty() {
            let label = format!("本题得分:{}分", problem.state.score_display.trim());
            *score_tags.entry(label).or_default() += 1;
        }
    }
    ...
    selected
        ...
        .map(|(index, problem)| {
            ...
            format!(
                ...
                director_problem_action_label(problem),
                answer_source_label_for_prompt(problem.state.answer_source),
...
fn director_problem_action_label(problem: &Problem) -> &'static str {
    if problem.state.answer_source == ProblemAnswerSource::AiReviewed
        && problem.state.review_status == ProblemReviewStatus::Accepted
    {
        "已写回AI参考答案"
    } else if problem.state.review_status == ProblemReviewStatus::Conflict {
        "证据冲突，已保留待人工确认"
    } else if problem.needs_ai_review() {
        "未安全确认，已保留待人工确认"
    } else if problem.state.answer_source == ProblemAnswerSource::ScoreInferred {
        "按得分推断保留"
    } else {
        "无需处理"
    }
...
fn answer_source_label_for_prompt(source: ProblemAnswerSource) -> &'static str {
    match source {
        ProblemAnswerSource::Standard => "标准答案",
        ProblemAnswerSource::UserTemporary => "用户临时答案",
        ProblemAnswerSource::ScoreInferred => "得分推断",
        ProblemAnswerSource::AiReviewed => "AI复核答案",
        ProblemAnswerSource::ManualReviewed => "人工确认答案",
    }
}
...
fn problem_set_stats(problems: &[Problem]) -> String {
    ...
    for problem in problems {
        ...
        for tag in visible_tags(&problem.tags) {
...
mod tests {
    ...
    fn review_prompt_treats_saved_answer_as_reference_only() {
        let problem = Problem {
            ...
            deck_info: None,
            images: vec![],
            state: crate::problem::ProblemState::default(),
        };
    ...
    fn structured_review_updates_answer_and_clears_pending_tags() {
        let mut problem = Problem {
            ...
            deck_info: None,
            images: vec![],
            state: crate::problem::ProblemState {
                user_answer: "B".into(),
                answer_source: crate::problem::ProblemAnswerSource::ScoreInferred,
                review_needed: true,
                review_status: crate::problem::ProblemReviewStatus::Pending,
                review_verdict: crate::problem::ProblemReviewVerdict::Wrong,
                review_confidence: crate::problem::ProblemReviewConfidence::Unknown,
                score_display: "0".into(),
            },
        };
        ...
        assert_eq!(
            ...
            crate::problem::ProblemAnswerSource::AiReviewed
        );
        assert_eq!(
            problem.state.review_status,
            crate::problem::ProblemReviewStatus::Accepted
        );
        assert_eq!(
            problem.state.review_verdict,
            crate::problem::ProblemReviewVerdict::Wrong
        );
    ...
    fn structured_review_without_reference_keeps_pending_review() {
        let mut problem = Problem {
            ...
            images: vec![],
            state: crate::problem::ProblemState {
                user_answer: "历史答案".into(),
                answer_source: crate::problem::ProblemAnswerSource::UserTemporary,
                review_needed: true,
                review_status: crate::problem::ProblemReviewStatus::Pending,
                review_verdict: crate::problem::ProblemReviewVerdict::Unknown,
                review_confidence: crate::problem::ProblemReviewConfidence::Unknown,
...
```"#,
        ...
        assert_eq!(
            problem.state.review_status,
            crate::problem::ProblemReviewStatus::Unknown
        );
    }

    #[test]
    fn structured_review_conflicting_with_known_wrong_keeps_pending() {
        let mut problem = Problem {
            id: "conflict".into(),
            prompt: "单选题\nA. 正确项\nB. 错误项".into(),
            answer: "B".into(),
            explanation: "当前作答得分 0 分，单选题可推断该作答错误。".into(),
            tags: vec!["待复核".into(), "作答错误".into()],
        ...
        let review = super::parse_structured_review(
            r#"{"verdict":"正确","reference_answer":"B","explanation":"历史作答看起来没问题。","confidence":"高"}"#,
        )
        .expect("review should parse");
        super::apply_structured_review(&mut problem, review);
        ...
        assert_eq!(
            problem.state.review_status,
            crate::problem::ProblemReviewStatus::Conflict
        );
        assert_eq!(
            problem.state.review_verdict,
            crate::problem::ProblemReviewVerdict::Correct
        );
    }

    #[test]
    fn structured_review_verdict_is_derived_from_reference_answer() {
        let mut problem = Problem {
            id: "derive".into(),
            prompt: "单选题\nA. 正确项\nB. 错误项".into(),
            answer: "B".into(),
        ...
        let review = super::parse_structured_review(
            r#"{"verdict":"正确","reference_answer":"A","explanation":"根据题干应选 A。","confidence":"高"}"#,
        )
        .expect("review should parse");
        super::apply_structured_review(&mut problem, review);
    ...
    fn compact_problem_set_prompt_uses_local_full_dataset_summary() {
        let mut problems = (0..284)
            ...
            .collect::<Vec<_>>();
        problems[37].answer = "C".into();
        problems[37].state.answer_source = crate::problem::ProblemAnswerSource::AiReviewed;
        problems[37].state.review_status = crate::problem::ProblemReviewStatus::Accepted;
        ...
        assert!(system.contains("本次分析已处理了什么"));
        assert!(system.contains("AI 复核写回题目的题号、题干和答案只能来自"));
        assert!(system.contains("会写回题库数据库"));
        assert!(prompt.contains("已读取全部题目（284 / 284）"));
        assert!(prompt.contains("程序处理动作摘要"));
        assert!(prompt.contains("AI复核写回明细"));
        assert!(prompt.contains("第 38 题「经济学需求供给题目 37"));
        assert!(prompt.contains("已写回 AI 复核答案 C"));
        assert!(prompt.contains("处理:无需处理"));
        assert!(prompt.contains("仅写回高置信且通过安全条件"));
    ...
    #[test]
    fn prefetched_explanation_preserves_existing_notes() {
        let merged = super::merge_prefetched_explanation("新的完整解析", "AI复核结果：错误");

        assert!(merged.starts_with("AI预生成解析："));
        assert!(merged.contains("新的完整解析"));
        assert!(merged.contains("原解析/复核记录"));
        assert!(merged.contains("AI复核结果：错误"));
    }
