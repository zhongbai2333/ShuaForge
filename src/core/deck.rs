...
impl PracticeDeck {
    ...
    pub fn current_or_next(&mut self) -> Option<&Problem> {
    ...
    pub fn skip_without_requeue(&mut self) {
        if let Some(problem) = self.current.take() {
            self.queue.retain(|queued| queued.id != problem.id);
        }
        self.current = self.queue.pop_front();
    }
...
mod tests {
    ...
    #[test]
    fn skip_without_requeue_removes_current_problem() {
        let mut deck = PracticeDeck::with_order(vec![problem("1", "a")], PracticeOrder::Sequential);

        deck.skip_without_requeue();

        assert!(deck.is_finished());
        assert_eq!(deck.stats().remaining, 0);
    }

    #[test]
    fn skip_without_requeue_removes_duplicate_current_problem_from_queue() {
        ...
        assert_eq!(deck.current().expect("next problem").id, "2");
        assert_eq!(deck.stats().remaining, 1);
        assert!(
            !deck
                .snapshot()
