...
- 导出题库 / 题组：可将本地题库或题组另存为可再次导入的 ZIP / JSON
- 答题历史
...
- 点击题库 / 题组卡片上的 `导出`，或在卡片右键菜单选择导出：保存可迁移的题库文件

## 导出题库 / 题组

桌面端可从题库主页导出：

- 单个题库：题库卡片上的 `导出`，或题库预览窗口中的 `导出题库`
- 题组：题组卡片上的 `导出`，或题组卡片右键菜单中的 `导出题组`

保存文件时可选择：

- `.zip`：推荐格式，包含 `problems.json`、图片资源和 README，可直接再次导入 ShuaForge。
- `.json`：纯 JSON 题库，适合备份、版本管理或人工编辑。

题组导出会把题组内所有题库合并为一个可导入题库包，并在 `deck_info` 中记录包含的题库明细。
